...
def generate_binned_data(data_path, step=0.005, kernel_size=4, ):
    ...
    normalized_time = np.arange(0, np.max(cam_ttl_times), step)#np.linspace(dlc_time[int(lift_times[ee])]- window_s, dlc_time[int(lift_times[ee])]+window_s, num =  199)#
    binned_spike_dict = []
    ...
    neuron_time = np.arange(0, end_time+step, step)
    # neuron_time = neuron_time[:-1]
    rhythm_interp = np.interp(neuron_time,xaxis2,avg_val)

    return smooth_spike_dict, rhythm_interp